...
class AsyncoreMulticastSocket(asyncore.dispatcher):
    def __init__(self, ip_address, port = 1024):
        server_address = ('', port)# Create the socket
        sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        # Bind to the server address# ?    sock.bind(server_address) ?

        group = socket.inet_aton(ip_address)
        mreq = struct.pack('4sL', group, socket.INADDR_ANY)
        sock.setsockopt(socket.IPPROTO_IP, socket.IP_ADD_MEMBERSHIP, mreq)
        sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)# Create the socket
        sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
        print("test Results")# Bind to the server address
        sock.bind(server_address)
    def handle_read(self):
        data, addr = self.recvfrom(2048)
    def writable(self):
        return False# don 't want write notifies
a = AsyncoreMulticastSocket(mc_ip_address)
a.loop()

class AsyncoreSocketUDPOld(asyncore.dispatcher):
...
def getDepthAndColorMat(pipeline):
    frames = frameset.pipeline.poll_for_frame()
    if not frames == None:
...
pipeline = openBagPipeline("~/Desktop/20181119_131946.bag")
...
def wait_for_multi_cast(ip_address, port):
    ...
    # Create the socket
    sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)# Bind to the server address
    sock.bind(server_address)
    ...
    def multi_cast_message(ip_address, port, message):
        multicast_group = (ip_address, port)# send out regular request
        for servers on network:
            sock = socket.socket(socket.AF_INET, socket.SOCK_DGRAM)
            sock.settimeout(0.2)# Set the time - to - live
            for messages to 1 so they do not go past the# local network segment.
    ttl = struct.pack('b', 1)
    sock.setsockopt(socket.IPPROTO_IP, socket.IP_MULTICAST_TTL, ttl)
    try: #Send data to the multicast group
    print('sending "%s"' % message + str(multicast_group))
    sent = sock.sendto(message.encode(), multicast_group)# Look
    for responses from all recipients
    while True:
